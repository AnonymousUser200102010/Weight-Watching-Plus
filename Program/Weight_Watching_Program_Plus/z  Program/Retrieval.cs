...
namespace WeightWatchingProgramPlus
{
	...
	internal class Retrieval : IRetrieval
	{
		...
		public Retrieval (IValidation valid, IStorage store, IMainForm mainForm, IPopup popup)
		{
		...
		public void ReadRegistry (string appendedRegistryValue, string registyValue)
		{
			...
			if (appendedRegistryValue.Equals(GlobalVariables.RegistryAppendedValue))
			{
				...
				FileInfo errorDumpFileInfo = new FileInfo("Files//Error.dmp");
				
				if (this.Validation.ValidateBackup(appendedRegistryValue, registyValue, filesBackupDirectory, this) && GlobalVariables.CreateBackups && (File.Exists("Files//Error.dmp") && DateTime.Compare(errorDumpFileInfo.LastWriteTime, DateTime.Now.AddMinutes(-6)) >= 0) )
				{
				...
				else if (GlobalVariables.Debug)
				{
					...
					Messages.Handler("The program has detected a recent crash! Therefore no backup can be made, and no attempt to make a backup has been made.", "Weight Watching Program +", true, 102400);
		...
		public string GetRegistryValueFromRegistry (string appendedRegistryValue, string registryValue, string registryIDKeyword)
		{
			...
			using (RegistryKey tempKey = Registry.LocalMachine.OpenSubKey(appendedRegistryValue + registryValue, true))
			{
				...
				var regValueParsed = (string)tempKey.GetValue(this.ParseRegistryKeyById(registryIDKeyword));
				
				return regValueParsed;
